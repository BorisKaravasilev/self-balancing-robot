...
*/
float Kp = 240; //100;
float Ki = 9; // Influence of the sum of integrated errors in the final equation
float Kd = 0.01;

const float SPEED_OF_INTEGRATION = 480; // Multiplier of the integrated error
...
const float targetAngleCalculated = -10.19;
const float targetAngleOffset = -1.06;
const float targetAngle = targetAngleCalculated + targetAngleOffset;
...
void setup() {
  ...
  // ******** Leg Servos
  rightLeg.initialize(SERVO_RIGHT_LEG_PIN, 0);
  leftLeg.initialize(SERVO_LEFT_LEG_PIN, 180);
...
void showPIDValuesOnScreen() {
  lcd.setCursor(0, 1);
  lcd.print(String(Kp, 0));
  
  lcd.setCursor(5, 1);
  lcd.print(String(Ki, 2));
  
  lcd.setCursor(11, 1);
  lcd.print(String(Kd, 3));
}
